...
void ProjetaLaser::process(){
    ...
    cv::Mat foto_virtual(cv::Size(640, 480), CV_64FC1, cv::Scalar(0));
    ...
    pcl::io::savePLYFileASCII("/home/grin/Desktop/testando.ply", *nuvem);
    ...
    for(size_t i = 0; i < nuvem->size(); i++){
        ...
        if(floor(X(0,0)) >= 0 && floor(X(0,0)) < foto_virtual.cols && floor(X(1,0)) >= 0 && floor(X(1,0)) < foto_virtual.rows){
            ROS_INFO("projetou alguem");
            cv::Vec3b cor;
            cor[0] = nuvem->points[i].r; cor[1] = nuvem->points[i].g; cor[2] = nuvem->points[i].b;
            foto_virtual.at<cv::Vec3b>(cv::Point(X(1,0), X(0,0))) = cor;
        }
    }
